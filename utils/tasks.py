...
task_manager = Task(
    ...
    description="\n".join([
        ...
        "   - **Arguments**:",
        "     - **file_path**: The path to the .wav audio file. Use forward slashes (/) or double backslashes (\\\\) for cross-platform compatibility.",
        "   - **Usage**: Use this tool FIRST when the user provides only an audio file path (.wav) as input.",
        "   - **Path Format**: When calling this tool, ensure paths use proper format:",
        "     - Good: 'audios/audio.wav' or 'audios\\audio.wav'",
        "     - Bad: 'audios\audio.wav' (single backslash)",
        "   - **Example**: If question is 'audios/temp_audio.wav', use transcribe_audio('audios/temp_audio.wav') to get the text content.",
